...
export default function ChatRoom() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`/api/chat/open?token=${encodeURIComponent(token)}`)
      .then(async (r) => {
        if (!r.ok) throw new Error(`Open failed (${r.status})`);
        return (await r.json()) as OpenResponse;
      })
      .then(setOpen)
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto bg-white border-x">
      <header className="px-4 py-3 border-b bg-ink text-white flex items-center justify-between">
        <div>
        ...
        <button
          ...
          disabled={ending}
          className="text-xs px-3 py-1 rounded-full border border-white/30 hover:bg-white/10 disabled:opacity-50"
        >
          End chat
        </button>
      </header>
      ...
      >
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button
            ...
            onClick={onCancel}
            disabled={ending}
            className="px-4 py-2 text-sm rounded-full border border-slate-200 text-slate-700 hover:bg-slate-50 disabled:opacity-50"
          >
            Cancel
          </button>
          <button
