...
- **Domain events on aggregates**: entities that inherit `AggregateRoot` (e.g. `ChatSession`, `ServiceRequest`) call `RaiseDomainEvent(...)` inside state-changing methods. `DomainEventScraper<AggregateRoot, IDomainEvent>` (wired in `Program.cs`) drains them at EF `SaveChanges` and enrols them in the durable outbox — same delivery guarantees as direct `bus.PublishAsync`, and it fires for any `SaveChanges` call (including outside a Wolverine handler, e.g. `ChatHub.EndChat`). Prefer raising over `bus.PublishAsync` from inside aggregates; orchestrators can still publish directly when no aggregate owns the event.
