...
- **Wolverine durable persistence (production)**: production (and any `Wolverine:Durable=true` env) persists scheduled messages + outbox in the `wolverine` Postgres schema. The schema is created by Wolverine itself, **not** by EF migrations — separate lifecycle from the `public` schema. Tests stay in-memory by default; `DevPipelineFixture` pins `Wolverine:Durable=false` explicitly to keep per-shard databases isolated.
