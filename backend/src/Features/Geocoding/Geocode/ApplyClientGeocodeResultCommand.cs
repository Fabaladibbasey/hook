...
public sealed record ApplyClientGeocodeResultCommand(
    string Phone,
    GeocodeResult Result,
    DateTimeOffset DraftStampedAt = default,
    string Reserved = "");

public sealed class ApplyClientGeocodeResultHandler(
    IClientRequestDraftRepository drafts,
    TimeProvider clock,
    ILogger<ApplyClientGeocodeResultHandler> logger)
{
    public async Task Handle(ApplyClientGeocodeResultCommand evt, IMessageBus bus, CancellationToken ct)
    {
